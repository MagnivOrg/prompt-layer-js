...
import { Pagination, TrackRequest } from "@/types";
...
const promptLayerAllPrompts = async (params?: Pagination) => {
  ...
  try {
    ...
    if (response.status !== 200) {
      console.warn(
        `WARNING: While fetching all prompts, PromptLayer had the following error: ${JSON.stringify(
          data
        )}`
      );
    }
    ...
    console.warn(
      `WARNING: While fetching all prompts, PromptLayer had the following error: ${e}`
    );
  }
};
